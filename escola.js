...
const isStudentRegistered = (studentId) =>
  students.findIndex((student) => student.id === studentId) !== -1;

const isClassRegistered = (classId) =>
  classes.findIndex((classAux) => classAux.id === classId) !== -1;

const isTeacherRegistered = (teacherId) =>
  teachers.findIndex((teacher) => teacher.id === teacherId) !== -1;

const getTeacherById = (teacherId) =>
  teachers.find((teacher) => teacher.id === teacherId);

const getClassById = (classId) =>
  classes.find((classAux) => classAux.id === classId);
...
function addStudent(studentId, name, email, age, classId) {
  if (isClassRegistered(classId) && !isStudentRegistered(studentId)) {
...
function addTeacher(teacherId, name, email, age) {
  isTeacherRegistered(teacherId)
    ? null
    : teachers.push({ id: teacherId, name, email, age });
}
...
function removeTeacher(teacherId) {
  if (isTeacherRegistered(teacherId)) {
    ...
    classes.forEach((classAux) => {
      classAux.teachers = classAux.teachers.filter(
        (teacher) => teacher.id !== teacherId
      );
    });
  }
...
function removeStudent(studentId) {
  if (isStudentRegistered(studentId)) {
    const { classId } = getStudentById(studentId);
    const studentIndex = students.findIndex(
      (student) => student.id === studentId
    );
    students.splice(studentIndex, 1);

    studentClass = getClassById(classId);
...
function removeClass(classId) {
  // Implemente a lógica.
  if (isClassRegistered(classId)) {
    const classAux = getClassById(classId);

    classAux.students.forEach((student) => (student.classId = null));

    const classIndex = classes.findIndex((classAux) => classAux.id === classId);
...
expect(
  teachers[0].name,
  "João",
  "Deve possuir professor João no array de professores"
);
expect(
  teachers[0].email,
  "joao@gmail.com",
  "Deve possuir e-mail do professor João"
);
...
expect(
  teachers.length,
  1,
  "Deve impedir de cadastrar professores com o mesmo ID"
);
...
expect(
  classes[0].name,
  "Turma 2022",
  "Deve possuir Turma 2022 no array de turmas"
);
expect(
  classes[0].teachers.length,
  1,
  "Deve possuir apenas 1 professor cadastrado para Turma 2022"
);
expect(
  classes[0].teachers[0].name,
  "João",
  "Deve possuir professor João na Turma 2022"
);
...
expect(
  classes[0].students[0].name,
  "Higor",
  "Deve possuir aluno chamado Higor"
);
...
expect(
  classes[0].students[1].name,
  "Pedro",
  "Deve possuir aluno chamado Pedro"
);
...
expect(
  classes[0].teachers.length,
  0,
  "Deve remover professor da turma que ele leciona"
);

removeStudent(1);
expect(students.length, 1, "Deve remover aluno de ID 1");
expect(
  classes[0].students.length,
  1,
  "Deve remover aluno da turma que ele faz parte"
);
